...
def message_gpt(msg, client, image_paths=[], model="gpt-4o", context_msgs=[], images_idx=-1, temperature=0):
    ...
    res = client.chat.completions.create(
        model=model, # <-- 关键修复 2：使用传入的 'model'
        messages=messages,
...
# --- 关键修复 3：添加 'model' 参数并传递 ---
def message_gpt_w_error_handle(msg, client, image_paths, context_msgs, model="gpt-4o", max_tries=3):
    unable = True
    temp = 0
...
# --- 关键修复 4：添加 'model' 参数并传递 ---
def retrieval_caption_generation(prompt, image_paths, gpt_client, model="gpt-4o", k_captions_per_concept=1, k_concepts=-1, decision=True, only_rephrase=False):
    if decision:
        ...
        if only_rephrase:
            # 传递 model
            rephrased_prompt = get_rephrased_prompt(prompt, gpt_client, model=model, image_paths=image_paths, context_msgs=context_msgs, images_idx=0)
...
# --- 关键修复 5：添加 'model' 参数并传递 ---
def get_rephrased_prompt(prompt, gpt_client, model="gpt-4o", image_paths=[], context_msgs=[], images_idx=-1):
    if not context_msgs:
