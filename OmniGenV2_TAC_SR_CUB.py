...
DATASET_CONFIG = {
    ...
    "output_path": f"results/{_rm}/{timestamp}/OmniGenV2_TAC_SR_CUB_{run_time}"
}
...
if __name__ == "__main__":
    ...
    for class_name_raw in tqdm(my_tasks):
        ...
        try:
            reference_specs = generate_knowledge_specs(class_name, client, args.llm_model, domain="cub")
        ...
        while retry_cnt < args.max_retries:
            ...
            diagnosis = taxonomy_aware_diagnosis(prompt, [current_image], client, args.llm_model, reference_specs=reference_specs, domain="cub")
        ...
        if not os.path.exists(final_success_path):
            ...
            if os.path.exists(current_image):
                diagnosis = taxonomy_aware_diagnosis(prompt, [current_image], client, args.llm_model, domain="cub")
    ...
    # Stop Monitor
    # Note: monitor is local to setup_system, but we need to access it here.
    # Since setup_system doesn't return it, we can't access it directly.
    # However, we can instantiate it here just to save plots if we assume it was running in background?
    # No, the instance holds the data.

    # I need to modify setup_system to return the monitor or make it global.
    # Or I can just instantiate it in main.
