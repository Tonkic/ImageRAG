'''
OmniGenV2_BC_MGR_Winoground.py
=============================
Configuration:
...
Usage:
  python OmniGenV2_BC_MGR_Winoground.py \
      --device_id 0 \
      --task_index 0 \
      --total_chunks 1 \
...
# --- 1. Argument Parsing ---
parser = argparse.ArgumentParser(description="OmniGenV2 + BC + MGR (Winoground)")
...
DATASET_CONFIG = {
    "output_path": "results/OmniGenV2_BC_MGR_Winoground",
...
def run_omnigen(pipe, prompt, input_images, output_path, seed):
    ...
    # OmniGen V2 Generation
    # Note: OmniGen V2 input_images should be a list of paths or PIL images
    # Here we pass paths if available

    # Construct prompt with image placeholders if needed
    # For MGR, we usually append images.
    # OmniGen format: "<img><|image_1|></img> prompt"
    ...
    if input_images:
        ...
        for i, img_path in enumerate(input_images):
            ...
            processed_images.append(Image.open(img_path)) # OmniGen accepts paths
        ...
        for suffix in ["0", "1"]:
            ...
            while retry_cnt < args.max_retries:
                ...
                retrieved_lists, _ = retrieve_img_per_caption(
                    ...
                    global_memory=global_memory # Pass Memory
                )
                ...
                next_path = os.path.join(DATASET_CONFIG['output_path'], f"{safe_name}_V{retry_cnt+2}.png")
                run_omnigen(pipe, prompt, [best_ref], next_path, args.seed + retry_cnt + 1)
