'''
Evaluation Script for: OmniGenV2 + TAC + MGR (CUB)
=======================================================
Metrics:
...
# --- 1. Args ---
# --------------------------------------------------
parser = argparse.ArgumentParser(description="Evaluate OmniGenV2_TAC_MGR_CUB")
...
DATASET_CONFIG = {
    ...
    "results_dir": "results/OmniGenV2_TAC_MGR_CUB"
}
...
def load_cub_tasks(config):
    ...
    with open(config['classes_txt']) as f:
        for line in f:
            ...
            folder_name = parts[1] # "001.Black_footed_Albatross"
            simple_name = folder_name.split('.', 1)[-1].replace('_', ' ') # "Black footed Albatross"
...
def main():
    ...
    print("\n" + "="*60)
    print(f"  EVAL REPORT: OmniGenV2 + TAC + MGR (CUB)")
    print("="*60)
