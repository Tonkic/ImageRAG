...
if __name__ == "__main__":
    exec_ablation(__file__, [
        (
            ...
            'out_root = f"results/{args.retrieval_method}/{dt.strftime(\'%Y.%-m.%-d\')}/OmniGenV2_IPC_AR_noLLMReinforce_{dt.strftime(\'%H-%M-%S\')}"',
        ),
        (
            'if tifa_use and (failed_features or diag.get("mismatch_cues")):',
            'if False:  # ABLATION: disable prompt reinforcement',
        ),
    ])
