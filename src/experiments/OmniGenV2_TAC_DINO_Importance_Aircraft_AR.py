...
parser.add_argument("--warmup_n_classes", type=int, default=0,
                    help="Run only first N classes for warmup self-check (0=disabled)")
...
# Qwen3-VL Generation Hyperparameters
parser.add_argument("--vl_greedy", action="store_true", default=False,
                    help="Use greedy decoding for image-text (VL) calls")
parser.add_argument("--vl_top_p", type=float, default=0.8)
parser.add_argument("--vl_top_k", type=int, default=20)
parser.add_argument("--vl_temperature", type=float, default=0.35)
parser.add_argument("--vl_repetition_penalty", type=float, default=1.0)
parser.add_argument("--vl_presence_penalty", type=float, default=1.5)
parser.add_argument("--vl_out_seq_length", type=int, default=16384,
                    help="Mapped to max_new_tokens for VL calls")

parser.add_argument("--text_greedy", action="store_true", default=False,
                    help="Use greedy decoding for text-only calls")
parser.add_argument("--text_top_p", type=float, default=1.0)
parser.add_argument("--text_top_k", type=int, default=40)
parser.add_argument("--text_repetition_penalty", type=float, default=1.0)
parser.add_argument("--text_presence_penalty", type=float, default=2.0)
parser.add_argument("--text_temperature", type=float, default=1.0)
parser.add_argument("--text_out_seq_length", type=int, default=32768,
                    help="Mapped to max_new_tokens for text-only calls")
parser.add_argument("--qwen_use_model_defaults", action="store_true", default=False,
                    help="Do not pass sampling hyperparameters; use model generation_config defaults")
parser.add_argument("--no_qwen_use_model_defaults", action="store_false", dest="qwen_use_model_defaults",
                    help="Explicitly disable model-default generation mode (recommended for stable structured output)")
...
def get_qwen_generation_kwargs(task_type="vl"):
    if args.qwen_use_model_defaults:
        return {}

    is_vl = task_type == "vl"
    greedy = args.vl_greedy if is_vl else args.text_greedy
    kwargs = {
        "temperature": args.vl_temperature if is_vl else args.text_temperature,
        "top_p": args.vl_top_p if is_vl else args.text_top_p,
        "top_k": args.vl_top_k if is_vl else args.text_top_k,
        "repetition_penalty": args.vl_repetition_penalty if is_vl else args.text_repetition_penalty,
        "max_new_tokens": args.vl_out_seq_length if is_vl else args.text_out_seq_length,
        "do_sample": not greedy,
    }

    presence_penalty = args.vl_presence_penalty if is_vl else args.text_presence_penalty
...
def importance_aware_input_interpreter(prompt, client, model="gpt-4o", domain="aircraft"):
    ...
    **STRICT OUTPUT CONTRACT (MUST FOLLOW):**
    ...
    def _validate_schema(result_obj):
        errors = []
        ...
        if not isinstance(result_obj.get("ambiguous_elements"), list):
            errors.append("ambiguous_elements_not_list")
        return errors

    def _build_default_result():
        entity_default = prompt.replace("a photo of a ", "").replace("a photo of ", "").strip() or prompt.strip()
        return {
            "high_importance": {
            ...
            "retrieval_query": f"a photo of a {entity_default}",
            "generation_prompt": prompt,
            "ambiguous_elements": [],
        }
    ...
    last_result = _build_default_result()
    ...
    for attempt in range(max_schema_retries + 1):
        ans_text = message_gpt(
            ...
            [],
            model=model,
            **get_qwen_generation_kwargs("text"),
        )
        ...
        try:
            ...
            root = _canon_obj(parsed)

            high_importance = _canon_obj(
                _pick(root, ["high_importance", "highimportance", "identified_elements"], {})
            )
            low_importance = _canon_obj(
                _pick(root, ["low_importance", "lowimportance", "creativity_fills"], {})
            )
            importance_weights = _canon_obj(
                _pick(root, ["importance_weights", "weights", "importance_weigths"], {})
            )

            entity = _pick(high_importance, ["entity", "main_subject", "main_subjects", "subject"], "")
            if not entity:
                entity = prompt.replace("a photo of a ", "").replace("a photo of ", "").strip() or prompt.strip()

            key_features = _pick(high_importance, ["key_features", "features", "attributes"], [])
            if isinstance(key_features, str):
                key_features = [key_features]
            if not isinstance(key_features, list):
                key_features = []
            key_features = [str(x).strip() for x in key_features if str(x).strip()]
            ...
            for key, default_val in lo_defaults.items():
                val = _pick(low_importance, [key], None)
                low_out[key] = str(val).strip() if val is not None and str(val).strip() else default_val
            ...
            for key, default_val in iw_defaults.items():
                iw_out[key] = _to_float(_pick(importance_weights, [key], None), default_val)

            retrieval_query = _pick(
                root,
                ["retrieval_query", "retrievalquery", "retrival_query", "retreival_query", "query"],
                None,
            )
            generation_prompt = _pick(
                root,
                ["generation_prompt", "generationprompt", "generation_promise", "detailed_prompt", "prompt"],
                None,
            )
            ambiguous_elements = _pick(root, ["ambiguous_elements", "ambiguous_element", "ambiguous"], [])
            if isinstance(ambiguous_elements, str):
                ambiguous_elements = [ambiguous_elements]
            if not isinstance(ambiguous_elements, list):
                ambiguous_elements = []
            ambiguous_elements = [str(x).strip() for x in ambiguous_elements if str(x).strip()]
            ...
            result = {
                ...
                "low_importance": low_out,
                "importance_weights": iw_out,
                "retrieval_query": str(retrieval_query).strip() if retrieval_query is not None else f"a photo of a {entity}",
                "generation_prompt": str(generation_prompt).strip() if generation_prompt is not None else prompt,
                "ambiguous_elements": ambiguous_elements,
            }

            schema_errors = _validate_schema(result)
            result["_parser_recovery_used"] = parser_recovery_used or bool(schema_errors)
            last_result = result
            ...
            if attempt < max_schema_retries:
                err_text = ", ".join(schema_errors[:10])
                preview = (ans_text or "")[:1200]
                current_msg = (
                    "Your previous JSON output is invalid. "
                    f"Schema errors: {err_text}. "
                    f"Previous output: {preview}\n\n"
                    "Now output ONLY valid JSON that strictly follows this instruction:\n"
                    f"{base_msg}"
                )
    ...
    last_result.setdefault("_parser_recovery_used", True)
    return last_result
...
def tac_spatial_diagnosis(prompt, image_path, client, model_name,
    ...
    diagnosis = taxonomy_aware_diagnosis(
        ...
        domain=domain,
        vlm_generation_kwargs=get_qwen_generation_kwargs("vl"),
        repair_retries=1,
        compact_output=True,
    )
    ...
    tac_status = diagnosis.get("status", "unknown")
    schema_warnings = diagnosis.get("schema_warnings", [])

    log(f"  [Step4-TAC] Score={score}, Taxonomy={taxonomy_status}, ErrorType={error_type}, Status={tac_status}")
    if schema_warnings:
        log(f"  [Step4-TAC] SchemaWarnings: {schema_warnings[:4]}")
...
if __name__ == "__main__":
    ...
    DATASET_CONFIG = {
        ...
        "output_path": f"results/{_rm}/{timestamp}/OmniGenV2_DualPath_AR_{run_time}",
    }
    ...
    warmup_mode = args.warmup_n_classes > 0
    if warmup_mode:
        original_count = len(my_tasks)
        my_tasks = my_tasks[:args.warmup_n_classes]
        print(f"[Warmup] Enabled: running {len(my_tasks)}/{original_count} classes for self-check.")
    ...
    def _update_warmup_diag(diagnosis_obj):
        if not warmup_mode or not isinstance(diagnosis_obj, dict):
            return
        warmup_stats["tac_calls"] += 1
        score_val = diagnosis_obj.get("final_score", 0.0)
        if isinstance(score_val, (int, float)):
            warmup_stats["scores"].append(float(score_val))
        critique = str(diagnosis_obj.get("critique", ""))
        status = str(diagnosis_obj.get("status", ""))
        if "JSON malformed, used field-level fallback." in critique or status == "partial_success":
            warmup_stats["tac_recovery"] += 1
    ...
    for class_idx, class_name in enumerate(tqdm(my_tasks, desc="5-Step Pipeline")):
        ...
        try:
            ...
            if warmup_mode:
                warmup_stats["classes"] += 1
                if interpretation.get("_parser_recovery_used", False):
                    warmup_stats["input_recovery"] += 1
    ...
    with open(final_log_path, "w") as f:
        ...
        if warmup_mode:
            tac_calls = warmup_stats["tac_calls"]
            tac_recovery = warmup_stats["tac_recovery"]
            input_recovery = warmup_stats["input_recovery"]
            cls_count = max(1, warmup_stats["classes"])
            tac_recovery_rate = (tac_recovery / tac_calls) if tac_calls > 0 else 0.0
            input_recovery_rate = input_recovery / cls_count
            avg_score = (sum(warmup_stats["scores"]) / len(warmup_stats["scores"])) if warmup_stats["scores"] else 0.0

            f.write("\n[Warmup Self-Check]\n")
            f.write(f"  Warmup classes: {warmup_stats['classes']}\n")
            f.write(f"  InputInterpreter recovery: {input_recovery}/{warmup_stats['classes']} ({input_recovery_rate:.2%})\n")
            f.write(f"  TAC recovery: {tac_recovery}/{tac_calls} ({tac_recovery_rate:.2%})\n")
            f.write(f"  TAC avg score: {avg_score:.4f}\n")

            if tac_recovery_rate > 0.20 or avg_score <= 0.0:
                f.write("  Warmup verdict: FAIL (建议先调参，不建议继续全量100类)\n")
            else:
                f.write("  Warmup verdict: PASS (可继续全量运行)\n")
    ...
    if warmup_mode:
        ...
        input_recovery_rate = input_recovery / cls_count
        avg_score = (sum(warmup_stats["scores"]) / len(warmup_stats["scores"])) if warmup_stats["scores"] else 0.0
        print(f"[Warmup] Input recovery rate: {input_recovery_rate:.2%} ({input_recovery}/{warmup_stats['classes']})")
        print(f"[Warmup] TAC recovery rate: {tac_recovery_rate:.2%} ({tac_recovery}/{tac_calls})")
        print(f"[Warmup] TAC avg score: {avg_score:.4f}")
        if tac_recovery_rate > 0.20 or avg_score <= 0.0:
            print("[Warmup] Verdict: FAIL -> 建议先调参再跑100类")
        else:
            print("[Warmup] Verdict: PASS -> 可继续跑100类")
