...
def dual_stage_retrieval(retrieval_query, retriever, client, model_name,
    ...
    # [Ablation] VAR Stage B is disabled. We blindly trust the base retriever's top-1 result.
    log(f"  [Step2-ABLATION] VAR verification disabled. Blindly selecting Top-1 candidate.")
    ...
    var_details = {
        ...
        "stage_b_passed": 1,
        "stage_b_results": [{"path": os.path.basename(candidates[0]), "base_score": candidate_scores[0], "vlm_answer": "ABLATION", "passed": True}]
    }

    valid_refs.append(candidates[0])
    best_ref_score = candidate_scores[0]
    log(f"    [#0] PASS ✓ → Selected as best ref ({os.path.basename(candidates[0])}) (Ablation)")

    for idx, (c_path, c_score) in enumerate(zip(candidates[1:], candidate_scores[1:]), start=1):
        valid_refs.append(c_path)
        log(f"    [#{idx}] PASS ✓ (backup: {os.path.basename(c_path)}) (Ablation)")
...
if __name__ == "__main__":
    ...
    DATASET_CONFIG = {
        ...
        "output_path": f"results/{_rm}/{timestamp}/OmniGenV2_Ablation_noVAR_AR_{run_time}",
    }
