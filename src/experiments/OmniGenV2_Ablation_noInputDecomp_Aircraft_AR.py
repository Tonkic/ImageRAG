...
def importance_aware_input_interpreter(prompt, client, model="gpt-4o", domain="aircraft"):
    ...
    """
    # [Ablation] Input Interpretation is disabled. Raw prompt is used for everything.
    print(f"[InputInterpreter-ABLATION] Skipping VLM decomposition, using raw prompt.")
    return {
        "high_importance": {
            "entity": prompt.replace("a photo of a ", "").replace("a photo of ", ""),
            "key_features": []
        },
        "low_importance": {
            "background": "",
...
if __name__ == "__main__":
    ...
    DATASET_CONFIG = {
        ...
        "output_path": f"results/{_rm}/{timestamp}/OmniGenV2_Ablation_noInputDecomp_AR_{run_time}",
    }
