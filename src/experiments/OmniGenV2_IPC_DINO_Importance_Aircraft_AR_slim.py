"""
OmniGenV2_IPC_DINO_Importance_Aircraft_AR_slim.py
==================================================
IPC 版本：Step 4 改用 identity_preservation_critic 替代 binary_critic，
其余与 TAC slim 版本完全相同（双 Client、ground-truth retry 等）。
"""
...
# Argument Parsing
# ==============================================================================
parser = argparse.ArgumentParser(description="OmniGenV2 IPC+DINO Importance Pipeline (slim, dual-client)")
...
parser.add_argument("--text_model",      type=str, default="Qwen/Qwen3-Omni-30B-A3B-Instruct")
parser.add_argument("--text_api_base",   type=str, default="https://api.siliconflow.cn/v1/")
...
# ==============================================================================
# Step 4: IPC Identity Diagnosis  (核心差异)
# ==============================================================================
def ipc_diagnosis(prompt, image_path, vl_client, vl_model,
    ...
    # Binary: same_identity → ACCEPT, else REJECT
...
def tifa_feature_eval(key_features, image_path, vl_client, vl_model, f_log=None):
    """对 key_features 逐条询问 VLM YES/NO，合并成一次调用。"""
    log = lambda m: (f_log.write(m+"\n") if f_log else print(m))
    if not key_features:
        return {}
    move_helpers_to_gpu(retrieval_device)
    try:
        ...
        resp = vl_client.chat.completions.create(
            ...
            messages=[{"role": "user", "content": [
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{img64}"}},
                {"type": "text", "text":
                    f"Examine this image and answer each question YES or NO.\n{questions}\n"
                    f"Reply ONLY: Q1: YES/NO, Q2: YES/NO, ..."}
            ]}],
            temperature=0.01, max_tokens=64)
        ans = (resp.choices[0].message.content or "").upper()
        log(f"  [TIFA] {ans[:200]}")
        import re
        results = {}
        for i, feat in enumerate(key_features):
            m = re.search(rf"Q{i+1}[:\s]+([A-Z]+)", ans)
...
#   IPC 版本额外合并 mismatch_cues 作为信号
#   仅在 max_retries > 2 时启用
# ==============================================================================
def llm_reinforce_prompt(gen_prompt, failed_features, text_client, entity_name,
                          mismatch_cues=None, f_log=None):
    """LLM 基于缺失特征 + IPC mismatch_cues 强化 generation_prompt。"""
    log = lambda m: (f_log.write(m+"\n") if f_log else print(m))
    if not failed_features and not mismatch_cues:
        return gen_prompt

    failed_str = "; ".join(f"'{f}'" for f in (failed_features or []))
    cues_str   = "; ".join(f"'{c}'" for c in (mismatch_cues  or [])[:4])

    cues_section = (
        f"\nIdentity mismatch diagnostics from IPC: {cues_str}." if cues_str else ""
    )
...
The generated image is MISSING these visual features: {failed_str or '(none from TIFA)'}.{cues_section}

Rewrite the prompt to STRONGLY EMPHASIZE all missing/mismatched features:
- Move critical features to the BEGINNING
...
def load_db():
    ...
    for ds in args.retrieval_datasets:
        ...
        if ds == 'aircraft':
            ...
            # ImageNet: fallback to scanning common root directories for JPEG/jpg images
            import glob
            ...
            if not paths:
                ...
                # Provide a dummy list so the retriever enters the batch loop and reads the cache
...
def setup_system(omnigen_device, vlm_device_map, shared_model=None, shared_proc=None):
    ...
    text_client = (UsageTrackingClient(
        ...
        if args.text_api_key else None)
    ...
    if text_client is None: text_client = vl_client
...
if __name__ == "__main__":
    ...
    with open(os.path.join(logs_dir, "run_config.txt"), "w") as f:
        f.write("=== IPC Pipeline Run Config ===\n")
    ...
    for cls_idx, class_name in enumerate(tqdm(my_tasks, desc="IPC Pipeline")):
        ...
        try:
            ...
            if tifa_use:
                ...
                feature_eval = tifa_feature_eval(key_features, v1_path, vl_client, vl_model, f_log=f_log)
            ...
            current_gen_prompt = gen_prompt  # 当前 prompt（可被 LLM 强化）
            ...
            if not accepted and args.max_retries >= 1:
                ...
                while retry_cnt < args.max_retries:
                    ...
                    # Step 5-A: LLM Prompt Reinforcement (max_retries > 2)
                    if tifa_use and (failed_features or diag.get("mismatch_cues")):
                        f_log.write(f"  [Step5-A] LLM reinforce; TIFA_failed={failed_features}, "
                                    f"IPC_cues={diag.get('mismatch_cues',[])[:3]}\n")
