...
def dual_stage_retrieval(retrieval_query, retriever, client, model_name,
    ...
    # ABLATION: noVAR
    # Stage B is skipped. Take Top-1 candidate directly.
    log(f"  [Step2-B ABLATION] Skipping VAR validation. Forcing Top-1 candidate.")
    valid_refs = [candidates[0]]
    best_ref_score = candidate_scores[0]
    var_details = {"stage_a_count": len(candidates), "stage_b_passed": 1, "stage_b_results": [{"path": os.path.basename(candidates[0]), "passed": True}]}
...
if __name__ == "__main__":
    ...
    DATASET_CONFIG = {
        ...
        "output_path": f"results/{_rm}/{timestamp}/OmniGenV2_Ablation_noVAR_{run_time}",
    }
