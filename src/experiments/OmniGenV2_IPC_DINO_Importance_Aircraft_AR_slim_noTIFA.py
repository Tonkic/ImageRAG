"""
OmniGenV2_IPC_DINO_Importance_Aircraft_AR_slim_noTIFA.py
=========================================================
Standalone noTIFA IPC pipeline.

相对 IPC slim 完整版，这个版本保留：
- Step 1 Importance-aware input interpreter
- Step 2 VAR retrieval
- Step 3 DINO-injected generation
- Step 4 IPC identity diagnosis
- Step 5 基于 IPC mismatch cues 的 LLM prompt reinforcement

显式移除：
- TIFA feature-level evaluation
- 基于 failed_features 的重试强化
"""
...
# Argument Parsing
# ==============================================================================
parser = argparse.ArgumentParser(description="OmniGenV2 IPC+DINO Importance Pipeline (slim noTIFA, dual-client)")
...
if __name__ == "__main__":
	...
	with open(os.path.join(logs_dir, "run_config.txt"), "w") as f:
		f.write("=== IPC noTIFA Pipeline Run Config ===\n")
	...
	for cls_idx, class_name in enumerate(tqdm(my_tasks, desc="IPC noTIFA Pipeline")):
		...
		try:
			...
			f_log.write(">>> STEP 4: IPC Identity Diagnosis (no TIFA)\n")
	...
	with open(os.path.join(logs_dir,"pipeline_summary.txt"),"w") as f:
		f.write(f"IPC noTIFA Pipeline | {elapsed:.1f}s | {len(my_tasks)} classes\n")
		write_run_metadata_block(f, run_metadata)
