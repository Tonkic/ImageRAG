...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ImageRAG Pipeline - Master Script")
    ...
    parser.add_argument("--dataset", type=str, default="CUB_train", help="Name of the RAG dataset folder inside ./datasets/")
    ...
    parser.add_argument("--out_path", type=str, default="results/CUB")
    ...
    # --- run_generation_cub.py 的参数 ---
    parser.add_argument("--device", type=int, required=True, help="GPU device ID (e.g., 0, 1, or 2)")
    parser.add_argument("--total_gpus", type=int, default=1, help="Total number of GPUs being used for the split")
    parser.add_argument("--classes_txt", type=str, default="CUB_200_2011/classes.txt", help="Path to classes.txt")
    ...
    # --- 2. 加载模型（真正只加载一次） ---
    print(f"[Device {args.device}] Loading OmniGen model (ONCE) onto device: {device} ...")
    pipe = OmniGenPipeline.from_pretrained("Shitao/OmniGen-v1")
    pipe.to(device)
    print(f"[Device {args.device}] OmniGen model loaded.")

    # --- 3. 加载 RAG 数据库路径（只需要一次） ---
    data_path = f"datasets/{args.dataset}"
    retrieval_image_paths = []
    print(f"[Device {args.device}] 正在递归扫描 {data_path} 中的所有图像...")
    for root, dirs, files in os.walk(data_path):
        for file in files:
            if file.endswith(('.png', '.jpg', '.jpeg', '.webp')):
                retrieval_image_paths.append(os.path.join(root, file))
    print(f"[Device {args.device}] 已找到 {len(retrieval_image_paths)} 张图像用于检索。")
    ...
    # --- 4. 加载并分配任务列表（只需要一次） ---
    print(f"[Device {args.device}] 正在从 {args.classes_txt} 加载类别列表...")
    ...
    for i, item in enumerate(all_items_to_generate):
        if i % args.total_gpus == args.device:
            items_for_this_gpu.append(item)

    print(f"[Device {args.device}] 总共 {len(all_items_to_generate)} 个类别。此设备将处理 {len(items_for_this_gpu)} 个。")

    # --- 5. 主循环（在同一个进程中运行）---
    for label_id, full_class_name in tqdm(items_for_this_gpu, desc=f"在 Device {args.device} 上生成图像"):

        # 动态生成 prompt 和 out_name
        current_prompt = f"a photo of a {full_class_name.split('.')[-1].replace('_', ' ')}"
        current_out_name = f"{label_id:03d}_{full_class_name}"

        print(f"\n--- [Device {args.device}] 正在运行: {current_prompt} ---")

        # --- (以下是 imageRAG_OmniGen.py 的原始 RAG 逻辑) ---

        out_txt_file = os.path.join(args.out_path, current_out_name + ".txt")
        f = open(out_txt_file, "w")
        f.write(f"prompt: {current_prompt}\n")

        # RAG 流程总是从 input_images 为空开始
        input_images = []
        k_concepts = 3
    ...
    print(f"--- [Device {args.device}] 已完成所有 {len(items_for_this_gpu)} 个任务 ---")